...
public class heartCheck extends Hospital implements CheckPro{
	...
	public void aiCheck(){
		...
		if (hp != null) {
			double give = hp.getHeartMoney(); // 获取用户CT的缴费情况
			String state = hp.getHeartState(); // 获取当前客户ct检查状态
	...
	public void docCheck(Hospital hp){
		String state = hp.getHeartState(); // 获取当前客户心电图检查状态
		if(state == null){
			System.out.println("\n望闻问切...");
			System.out.println("\n...........");
			System.out.println("\n经过我的问诊我建议您去缴费做心电图检查后拿结果来我这开药");
			hp.setHeartState("待检查"); // 问诊完修改检查状态为待检查
			...
			hp.setHeartState("已开药"); // 开完药修改检查状态为已开药
		}else if("已开药".equals(state)){
